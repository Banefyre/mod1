...
void Shader::initialize(std::string strVertexFile, std::string strFragmentFile)
{
	...
	if ( _vertexShaderId || _fragmentShaderId || _shaderProgramId )
		destroy();
	...
	glShaderSource(_vertexShaderId, 1, &szVShader, nullptr);
	glShaderSource(_fragmentShaderId, 1, &szFShader, nullptr);
...
GLint Shader::getVariable(std::string strVariable)
{
	...
	return glGetUniformLocation(_shaderProgramId, strVariable.c_str());
}
...
void Shader::destroy()
{
	...
	if(_shaderProgramId)
	{
		glDeleteShader(_shaderProgramId);
		_shaderProgramId = 0;
	}
}
